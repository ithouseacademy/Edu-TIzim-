...
import { api, storeEmployee } from "../api"
...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.is_admin) {
        await api.logout()
        setError("Sizga admin paneliga kirish ruxsati yo'q")
        setLoading(false)
        return
      }
      storeEmployee(res.employee, true)
      navigate("/today")
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-white text-[#111827] select-none relative overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-[380px] mx-auto px-5 py-6 flex flex-col min-h-screen sm:min-h-0">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-[12px] mt-2">Guruhlar, bugungi darslar va to'lovlarni kuzatib boring</p>
        </div>
